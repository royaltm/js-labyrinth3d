...
function serialize() {
  ...
  var i = 0;
  for (var len = exits.length; i < len; ++i) {
    if (i % 2 === 1) {
      temp |= (exits[i] & 7) << 3;
      res += chars[temp];
    }
...
function loadFrom(data) {
  ...
  var header = ref[0];
  var body = ref[1];
  var dimensions = header.split(',').map(Number);
  var cols = dimensions[0];
  var rows = dimensions[1];
  var floors = dimensions[2];

  if (dimensions.length !== 3 || !dimensions.every(function (v) { return isFinite(v) && v % 1 === 0; })) {
  ...
  for(var i = 0, n = 0; n < len; ++i, ++n) {
    var temp = chars.indexOf(body[i]);
...
Wall$1.prototype.serialize = serialize;
Wall$1.loadFrom            = loadFrom;
